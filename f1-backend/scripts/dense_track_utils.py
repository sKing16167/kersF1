...
def get_dense_track(svg_path):
    ...
    if arrow_pts:
        ...
        # 50 samples forward vs backward
        d_fwd = np.hypot(x_fwd[50] - arrow_center[0], y_fwd[50] - arrow_center[1])
        d_bwd = np.hypot(x_fwd[-50] - arrow_center[0], y_fwd[-50] - arrow_center[1])
        if d_bwd < d_fwd:
            forward = False
